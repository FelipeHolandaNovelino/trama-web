...
function RelationshipChips({ relationships }) {
  const visibleRelationships = relationships.slice(0, 5)
  const hiddenCount = relationships.length - visibleRelationships.length

  if (relationships.length === 0) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-dashed border-slate-300 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-400">
        <UsersIcon />
        Sem relacionamentos
      </span>
    )
 ...
 * Os relacionamentos aparecem de forma discreta porque agora alimentam
 * as opções de Relações ao criar blocos na timeline.
 */
    ...
    <header className="rounded-3xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {onBackToPatients && (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
              >
                <ArrowLeftIcon />
                Pacientes
              </button>
        ...
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-violet-800 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-900 xl:w-auto"
        >
          <PlusIcon />
          Nova sessão
        </button>
